...
class Mappress_Map extends Mappress_Obj {
	...
	function to_html() {
		// Exclude a few fields that don't need to be in the component (pois are determined later)
		$vars = array_diff_key(get_object_vars($this), array('otitle' => '', 'pois' => '', 'status' => '', 'title' => ''));
		$vars = (object) array_merge($vars, $this->get_layout_atts());

		// Convert center from object to string
	...
	*/
	function display_web_component($atts = null, $in_iframe = false) {
		...
		if (Mappress::$options->iframes && !$in_iframe) {
			...
			// Width + height attributes are required for Google AMP
			$atts = $this->get_layout_atts();
			$layout_atts = Mappress::to_atts($atts);

			return "<iframe $layout_atts class='mapp-iframe' src='$url' scrolling='no' loading='lazy'></iframe>";
		}
	...
	function get_layout_atts() {
		$vars = (object) array();
		$vars->id = $this->name;
		$vars->class = 'mapp-layout';
		$vars->class .= (Mappress::$options->iframes) ? ' mapp-has-iframe' : '';

		$alignment = ($this->alignment) ? $this->alignment : Mappress::$options->alignment;
		if ($alignment) {
			$vars->class .= ' align' . $alignment;
			$vars->class .= ' mapp-align-' . $alignment;
		}

		$dims = $this->get_dims();
		$vars->style = sprintf("width: %s;", ($this->alignment == 'full') ? "auto" : $dims->width);
		$vars->style .= (stristr($dims->height, '%')) ? sprintf("aspect-ratio: 100/%d;", (int) $dims->height)  : "height: {$dims->height};";
		return (array) $vars;
	}
